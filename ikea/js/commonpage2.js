...
$(window).ready(function(){
  ...
  setTimeout(function(){
    textHeight[0].classList.add('ani')},1000
  )
...
for(let textIndex = 1; textIndex < textHeight.length; textIndex++){
  window.addEventListener("scroll", function(){
    if(scrollY > 300+(textIndex-1)*450){
      textHeight[textIndex].classList.add('ani');
    }
    // if(3050 < scrollY){
  ...
  function slideRotate(num){
    slideBox.style.transform = ("rotateY(" + num * slideDeg + "deg)")
    slideIdx = num;
  }
  

  nextDiv.addEventListener("click",function(){
    slideRotate(slideIdx - 1)
  })
